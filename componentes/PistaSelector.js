...
export default function FormularioReserva({ navigation }) {
  ...
  // Generamos horas entre 08:00 y 22:00
  ...
  useEffect(() => {
    const fetchPistas = async () => {
      ...
      try {
        const res = await fetch('http://localhost:3001/pistas/disponibles'); // endpoint que devuelve solo disponibles
    ...
    <View style={styles.container}>
      ...
      <View style={styles.pickerWrapper}>
        {loadingPistas ? (
          ...
          <Picker enabled={false} style={styles.picker} dropdownIconColor="#1976D2">
            <Picker.Item label="No hay ninguna pista disponible" value="" />
          </Picker>
        ) : (
          <Picker
