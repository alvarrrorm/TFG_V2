...
const MisReservas = () => {
  ...
  const fetchReservas = async () => {
    try {
      ...
      const response = await fetch(`https://tfgv2-production.up.railway.app/api/reservas?nombre_usuario=${encodeURIComponent(nombreUsuario)}`);
  ...
  const cancelarReserva = async (idReserva) => {
    try {
      ...
      const response = await fetch(`https://tfgv2-production.up.railway.app/api/reservas/${idReserva}`, {
        method: 'DELETE'
      });
